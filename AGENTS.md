...
- **CRITICAL: the build is OFFLINE sqlx** — `query!` macros use the committed
  ...
  event store (sqlx 0.9) coexist with no live DB. Stale cache → regenerate
  against a disposable local database: set `SQLX_OFFLINE="false"`,
  `sqlx db reset -y --no-dotenv --database-url
  "sqlite:///ABS/path/issuance.db"`,
  `cargo sqlx prepare --database-url
  "sqlite:///ABS/path/issuance.db" -- --all-targets`,
  restore `"true"`, commit.
