...
impl CorporateActionFeed {
    ...
    async fn consume_connection(
        ...
        if let Some(cursor) = cursor {
            request = request.query(&[("since_id", cursor.as_str())]);
        } else {
...
pub(crate) async fn apply_mutation(
    ...
    {
        let next = mutation.event_id.clone();
        sqlx::query(
            "
...
mod tests {
    ...
    async fn consume_sse(
        ...
        feed.consume_connection(cursor).await.unwrap();
    }
    ...
    async fn applies_completed_frames_before_returning_a_poison_error() {
        ...
        let error = feed.consume_connection(None).await.unwrap_err();
    ...
    #[tokio::test]
    async fn poison_without_an_event_id_blocks_first_install_restart() {
        let harness = TestHarness::new().await;
        let server = MockServer::start();
        server.mock(|when, then| {
            when.method(GET).path("/corporate-actions");
    ...
    async fn replay_connection_accepts_the_anchor_before_new_events() {
        ...
        feed.consume_connection(Some(&cursor_mutation.event_id)).await.unwrap();
    ...
    async fn projection_transaction_rolls_back_cursor_when_schedule_write_fails()
     {
        ...
        let cursors: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM corporate_action_cursor")
                ...
                .await
                .unwrap();
        assert_eq!((mutations, schedules, cursors), (0, 0, 0));
    }
