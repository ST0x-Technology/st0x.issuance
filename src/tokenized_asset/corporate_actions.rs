...
impl CorporateActionsSync {
    pub(crate) fn new(
        ...
        Self { alpaca, scheduler, pool, notifier }
    }
...
pub(crate) fn spawn_corporate_actions_sync(mut sync: CorporateActionsSync) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            match sync.sync_and_notify(Utc::now()).await {
                Ok(summary) => {
                    debug!(target: "tokenized_asset",
...
mod tests {
    ...
    async fn sync_once_arms_supported_and_skips_the_rest() {
        ...
        let mut sync = CorporateActionsSync::new(
            alpaca,
            scheduler,
            harness.pool,
            Arc::new(CapturingLifecycleNotifier::default()),
        );
