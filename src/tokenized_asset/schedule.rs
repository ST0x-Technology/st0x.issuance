...
const CORPORATE_ACTION_ALIGNMENT_MAX_ATTEMPTS: u32 = 1_000_000;
...
pub(crate) enum CorporateActionFreezeError {
    ...
    #[error("failed to dispatch corporate-action freeze alignment: {0}")]
    Dispatch(#[from] Box<AggregateError<LifecycleError<Underlying>>>),
}
    ...
    async fn perform(
        ...
        if should_hold {
            ctx.underlying_store
                ...
                .await
                .map_err(|source| {
                    CorporateActionFreezeError::Dispatch(Box::new(source))
                })?;
        }
...
pub(crate) async fn reset_orphaned_freeze_schedule_jobs(
    ...
    let dead_alignments: Vec<String> = sqlx::query_scalar(
        "
        SELECT idempotency_key
        FROM Jobs
        WHERE
...
pub(crate) async fn vacuum_terminal_freeze_schedule_jobs(
    ...
    if !dead_alignments.is_empty() {
        error!(target: "asset", jobs = ?dead_alignments,
            "Corporate-action freeze alignments died without applying"
        );
    }
