...
impl LifecycleNotifier for TelegramNotifier {
    async fn notify(&self, notification: &LifecycleNotification) {
        if let Err(error) = self.send(notification).await {
            log_delivery_failure(notification, &error);
        }
    }
    ...
    async fn deliver(
        ...
        match self.send(notification).await {
            ...
            Err(error) if error.is_retryable() => {
                Err(LifecycleNotificationError::new(error))
            }
            Err(error) => {
                log_delivery_failure(notification, &error);
                Ok(())
...
fn log_delivery_failure(
    ...
    match notification {
        ...
        | LifecycleNotification::FreezeApplied { underlying }
        ...
        LifecycleNotification::CorporateActionsSyncFailed => tracing::error!(
            target: "notifications",
            event = "notification_delivery_failed",
            notification_kind = notification.kind().as_str(),
...
pub(super) enum TelegramDeliveryError {
...
impl TelegramDeliveryError {
    fn is_retryable(&self) -> bool {
        match self {
            Self::Request(_) | Self::Response(_) => true,
            Self::ApiStatus(status) => {
                *status == StatusCode::TOO_MANY_REQUESTS
                    || status.is_server_error()
            }
            Self::ApiRejected => false,
        }
    }
}
...
mod tests {
    ...
    use crate::notifications::{
        LifecycleNotificationJobCtx, SendLifecycleNotification,
    };
    ...
    #[tokio::test]
    async fn durable_job_returns_delivery_failure_for_retry() {
        let server = MockServer::start_async().await;
        server
            .mock_async(|when, then| {
        ...
        let job = SendLifecycleNotification { notification: notification() };
        let ctx = LifecycleNotificationJobCtx { notifier };

        assert!(job.perform(&ctx).await.is_err());
    }

    #[traced_test]
    #[tokio::test]
    async fn durable_job_completes_after_permanent_api_rejection() {
        let server = MockServer::start_async().await;
        server
            .mock_async(|when, then| {
                when.method(POST).path("/bot123:abc/sendMessage");
        ...
        assert!(job.perform(&ctx).await.is_ok());
    ...
    #[tokio::test]
    async fn durable_job_retries_rate_limit_responses() {
        let server = MockServer::start_async().await;
        server
            .mock_async(|when, then| {
                when.method(POST).path("/bot123:abc/sendMessage");
    ...
    async fn api_rejection_is_structured_and_does_not_log_the_response_body() {
        ...
        assert!(logs_contain_at!(
            Level::ERROR,
            &["notification_delivery_failed", "freeze_applied", "AAPL", "400"]
        ));
        assert!(!logs_contain_at!(
            Level::ERROR,
    ...
    #[traced_test]
    #[tokio::test]
    async fn redemption_failure_logs_request_and_underlying() {
        let server = MockServer::start_async().await;
        server
            .mock_async(|when, then| {
                when.method(POST).path("/bot123:abc/sendMessage");
        ...
        let notification = LifecycleNotification::RedemptionHeld {
