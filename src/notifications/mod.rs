...
/// Removes terminal lifecycle-notification jobs while preserving retryable
/// pending, running, and failed rows.
...
mod tests {
    ...
    #[tokio::test]
    async fn startup_maintenance_requeues_orphans_and_vacuums_terminal_rows() {
        ...
        queue
            ...
            .unwrap();
        queue
            .push_with_idempotency_key(job, "notification:terminal")
            .await
            .unwrap();
