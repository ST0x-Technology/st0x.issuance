...
impl<ES: EventStore<Redemption>> JournalManager<ES> {
    ...
    pub(crate) async fn recover_alpaca_called_redemptions(
        ...
        debug!(underlying = %underlying, "Starting recovery of AlpacaCalled redemptions");
        ...
        if stuck_redemptions.is_empty() {
            debug!(underlying = %underlying, "No AlpacaCalled redemptions to recover");
            return;
        }
        ...
        debug!(underlying = %underlying, "Completed recovery of AlpacaCalled redemptions");
    }
