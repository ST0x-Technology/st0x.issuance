...
#[derive(Debug, Clone)]
pub struct ContractWalletId(String);
...
impl FireblocksConfig {
    /// # Errors
    ///
    /// Returns an error if the secret file cannot be read.
    pub fn new(
...
/// # Errors
///
/// Returns an error on an empty input or a malformed `chain:asset` pair.
pub fn parse_chain_asset_ids(
    s: &str,
