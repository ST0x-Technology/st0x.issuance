...
pub(crate) use orchestrator::{
    MintAuthorization, MintedLogQuery, MintedLogScan, OrchestratorBurnParams,
    OrchestratorBurnReadiness, OrchestratorBurnResult, OrchestratorMintParams,
    OrchestratorMintResult, OrchestratorMintedLog, OrchestratorRevertReason,
};
...
pub(crate) trait VaultService: Send + Sync {
    ...
    ///
    /// `query` is the authoritative signed tuple the checks read; callers
    /// build it with `nonce` taken from the delivered authorization, whose
    /// remaining contribution here is the signature.
...
/// orchestrator burning what it pulled). Verification fails closed unless the
/// pull total equals the burn total, so a transaction that only partially
/// matches the orchestrator shape (or matches the other mode's shape) is
/// rejected as [`VaultError::NotABurn`].
