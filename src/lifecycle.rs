...
pub(crate) enum Lifecycle<T, E> {
    ...
    Failed { error: LifecycleError<E>, last_valid_state: Option<Box<T>> },
}
...
impl<T, E: Display> Lifecycle<T, E> {
        ...
        F: FnOnce(&Ev) -> Result<T, LifecycleError<E>>,
    {
        match self {
            Self::Uninitialized
            | Self::Failed { last_valid_state: None, .. } => match f(event) {
                Ok(new_state) => Self::Live(new_state),
                Err(err) => {
                    error!("Lifecycle failed during initialization: {err}");
                    Self::Failed { error: err, last_valid_state: None }
                }
            },
            Self::Live(current) => {
        ...
        F: FnOnce(&Ev) -> Result<T, LifecycleError<E>>,
    {
        match &self {
            Self::Uninitialized
            | Self::Failed { last_valid_state: None, .. } => {
                self.initialize(event, f)
            }

            Self::Live(_) | Self::Failed { last_valid_state: Some(_), .. } => {
                self
            }
        }
    }
...
mod tests {
    ...
    fn transition_on_active_succeeds() {
        let state: Lifecycle<TestState, TestError> =
            Lifecycle::Live(TestState { value: 42 });

        let state = state.transition(
            &TestEvent::Increment { amount: 10 },
            |ev, cur| match ev {
                TestEvent::Increment { amount } => {
                    Ok(TestState { value: cur.value + amount })
                }
    ...
    fn transition_on_uninitialized_corrupts_with_none() {
        ...
        let state = state
            .transition(&TestEvent::Increment { amount: 10 }, |_, _| {
                Ok(TestState { value: 100 })
            });

        let Lifecycle::Failed { error, last_valid_state } = state else {
    ...
    fn or_initialize_skipped_after_successful_transition() {
        let state: Lifecycle<TestState, TestError> =
            Lifecycle::Live(TestState { value: 10 });
    ...
    fn or_initialize_skipped_after_real_transition_error() {
        ...
        assert!(
            matches!(error, LifecycleError::Custom(TestError(msg)) if msg == "real error")
        );
    ...
    fn multiple_transitions_accumulate() {
        let mut state: Lifecycle<TestState, TestError> =
            Lifecycle::Live(TestState { value: 0 });
