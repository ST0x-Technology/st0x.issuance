...
| Fireblocks tx (we author)    | bare `String` (`external_tx_id`)   | 🐛     | proposed `ExternalTxId` ADT (see below)                                         |
...
graph LR
  ...
  ETIBug["🐛 external_tx_id: String"]:::bug --> FbTx
...
We author this string and send it to Fireblocks for idempotency. Today it's just
`String`, with formats spread across `mint/mod.rs`, `vault/service.rs`, and
`fireblocks/vault_service.rs`, plus parsing helpers like
