...
grants on the vault's authorizer (omitting `--asset` checks the assets whose
configured `vault_mode` resolves to orchestrator — the assets actually cutting
over). The gate is the **exit status**: zero only when every check passes, so
the RAI-1222 pre-checks gate on the exit code for the asset being cut over;
`Overall: READY` is the human-readable rendering of the same verdict.
