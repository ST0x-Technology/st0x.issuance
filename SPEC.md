...
- A `Failed` state reached from `BurnIntended` or `BurnSubmitted` retains the
  transaction's full burn lifecycle context and persisted receipt plan as
  historical evidence. `ReplaceExhaustedDeadBurn` may reuse that plan only when
  receipt inventory can reserve it before signing. If released allocations were
  consumed, vault-direct recovery plans and reserves current inventory without
  repeating the Alpaca call. `BurnResumed` carries the retained context through
  `Burning` until replacement succeeds or another terminal failure records it
  again. Pre-Alpaca failures carry no burn context.
...
- `RecordBurnTxSubmitted` / `RecordOrchestratorBurnSubmitted` - Record the burn
  ...
  `BurnIntended`; each command is bound to the exact expected transaction hash
  and becomes an idempotent no-op if a replacement is now current or the
  redemption has advanced, so an at-least-once job rerun is safe.
...
- `ReplaceExhaustedDeadBurn` - Re-check a manager-supplied operator proof
  ...
  block must be canonical at or below the finalized head. Vault-direct recovery
  receives either no plan override when the retained allocations were
  successfully re-reserved, or a freshly reserved plan whose burn and dust
  totals are bound to the redemption's persisted Alpaca quantities.
...
- `Reprocess { issuer_request_id, metadata }` - Reset a failed redemption back
  ...
  `Detected` event. Pre-Alpaca `Failed` states have no retained burn context, so
  reprocessing cannot source this metadata from the terminal state. Emits
  `Reprocessed` with the metadata, previous state name, and timestamp for audit
  trail. The existing recovery logic then picks it up naturally from `Detected`
  state.
...
- `BurnRecoveryExhausted` - Records that the automatic recovery budget is spent,
  ...
  aggregate unresolved. An ambiguous transaction keeps its receipt reservation;
  a finalized revert may already have released it. Its first persistence emits
  the single actionable operator error. Later periodic passes continue read-only
  exact-hash classification and may record a mined transaction, but perform no
  new signing or unbounded broadcast side effects. The admin recovery endpoint
  may authorize one fresh replacement per request only after it proves that the
  exact current transaction is dead.
- `ManualBurnReplacementAuthorized` - Records an operator-authorized replacement
  after automatic recovery is exhausted. It carries a unique recovery id plus
  the old and replacement transaction hashes and nonces. The same CQRS command
  also emits `BurnIntended`, whose plan is either the successfully re-reserved
  retained plan or a fresh vault-direct plan. The authorization and replacement
  identity therefore commit atomically. Recovery may use this durable
  authorization only to enqueue the exact persisted replacement after a queue
  failure or restart. It may not sign another transaction or spend another
...
4. With no receipt, if the finalized account nonce for `W` is greater than `N`,
   the transaction is **provably dead** and can never land. This normally
   permits signing a replacement at a fresh nonce. Automatic recovery and
   orchestrator-mode exhausted recovery preserve the persisted transaction's
   destination, value, and calldata exactly. Exhausted vault-direct recovery may
   instead use freshly planned calldata after it proves and reserves the
   retained plan is no longer available. A fresh nonce is the authoritative
   pending account nonce so it cannot collide with another live transaction from
   the signing wallet.
...
rebroadcasting the exact persisted bytes is the additional death proof: recovery
durably records it against `(H, N)`, and every later pass still checks
`receipt(H)` before using that observation to select replacement. A definitive
protocol rejection of the exact persisted bytes is the same kind of death proof:
the transaction never entered the pool, recovery durably records it against
`(H, N)`, and every later pass still checks `receipt(H)` before using that
observation to select replacement. A server or transport error followed by a
negative hash lookup remains ambiguous because load-balanced RPC backends can
disagree about mempool contents. A missing block number, mismatched receipt
hash, provider error, timeout, signer that differs from `W`, or any other
identity/RPC uncertainty is unclassified and fails closed: the old transaction
remains live, no replacement is signed, and its reservation remains held.
Same-nonce fee replacement is not supported.
...
permits exact transaction replacement. If reservation fails because those
allocations are no longer available, the manager plans the persisted
`alpaca_quantity` plus `dust_quantity` against current receipt inventory and
reserves that fresh plan before signing. The aggregate binds the new plan's burn
and dust totals to those persisted quantities and prepares new calldata. The
historical plan remains in the old events. Insufficient current inventory
refuses recovery without signing. Orchestrator mode has no per-receipt
reservation and continues to replace the exact persisted calldata.

If an already-authorized replacement retained in `Failed` is `Mined`, the
operator request confirms it inline and reports completion only after the
completion event is recorded; a transient confirmation error reports deferred
recovery. If it is `FinalizedReverted`, the failure is recorded and a fresh
replacement may be authorized. An unfinalized `Reverted` result signs nothing. A
durable nonce-too-low observation is not sufficient for this manual path; a new
...
`SubmitBurnJob` with a transaction-specific idempotency key. If queue dispatch
fails after the commit, the endpoint reports partial success. If the committed
authorization cannot be reloaded, the endpoint returns a distinct HTTP 200
committed-inspection-required response with the recovery id and old transaction
...
`OrchestratorBurnSubmitted` via `RecordOrchestratorBurnSubmitted`; the submit
job holds the network wallet lock through its post-broadcast state check and
result command, so a manual replacement cannot interleave there;
...
reviews the exact effect over HTTP before committing with `execute=true` rather
than reading the process log. In `outcome.plan`, `path` is `internal` or
`external` and `underlying` is the equity symbol, so an operator can identify
the target burn; the `bind` object carries the receipt id, shares, vault,
...
four per-tier verifiers share one HTTP connection pool but each caches its own
audience-scoped key set and keeps its own refresh floor; a verifier keeps
serving its retained keys (a stale set included) through a fetch outage, since a
stale Google key is still Google's, and a refresh prompted by an unrecognized
key id is throttled to one outbound fetch per minute per tier. Verification
fails with a retryable 503 when the endpoint is unreachable and no usable key is
...
- **Post-Alpaca failures** (`AlpacaCalled` event exists): Polls Alpaca to verify
  ...
  request.
- **Exhausted persisted burn** (`BurnRecoveryExhausted`, or five recorded
  automatic attempts): Does not resume or repeat the Alpaca call. The persisted
  burn proves that the journal completed. The manager locks the burn's network
  wallet, reloads the aggregate, and classifies the exact signed transaction. A
  `ProvablyDead` transaction, or a `FinalizedReverted` transaction retained in
  ...
  the same bytes without signing.
...
- `200`: Recovery initiated. For an exhausted burn, the response includes the
  old and replacement hashes and nonces. It states whether the submit job was
  queued or deferred after the replacement committed.
- `404`: Aggregate not found
- `409`: Aggregate already completed or closed (cannot recover), or
  `burn_replacement_already_queued` when an existing manual authorization has an
  active submit job. In the latter case, wait and inspect the aggregate again;
  do not request another signature.
- `422`: Aggregate in a state that cannot be recovered, Alpaca journal not
  completed, prior burn not confirmed reverted, malformed persisted identity,
  non-dead exhausted burn, or another signer intent blocks the wallet. Manual
  refusal responses include a stable error code and message.
- `500`: Local persistence or queue failure, or a successfully mined burn
  receipt is missing its block number. A committed manual replacement uses the
  `200` partial-success response instead of a retryable hard failure.
- `502`: Failed to poll Alpaca, classify the exhausted transaction through the
  RPC, or execute post-Alpaca burn recovery.
