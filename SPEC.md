...
- `Recover { issuer_request_id, mode }` - Recover a mint stuck in an incomplete
  ...
  failures can be retried with freshly prepared transactions
...
- `CloseMint { issuer_request_id, reason, acknowledged_unresolved_mint_nonce }` -
  Admin-close a mint that cannot be automatically recovered, recording an
  operator `reason`. Valid from `PendingJournal`, `JournalConfirmed`, `Minting`,
  `TxSubmitted`, or `MintingFailed` — **not** from `CallbackPending`, since
  reaching `CallbackPending` means an on-chain mint
  (`TokensMinted`/`OrchestratorTokensMinted`/`ExistingMintRecovered`/
  `OrchestratorMintRecovered`) is already recorded in this aggregate's own
  history; an economically committed mint must be resolved through the success
  path (`SendCallback`), never hidden by `CloseMint`. From the remaining,
  pre-mint states, the handler additionally checks for a matching on-chain mint
  before closing (vault-direct: no receipt found for this `issuer_request_id`;
  orchestrator: no `Minted` log matches `(to, nonce, token, amount)` — see
  "Nonce" below) — if one is found, `CloseMint` is rejected and the operator
  runs `Recover` to reconcile the already-completed mint instead. **That absence
  check alone is not sufficient whenever the aggregate's history holds a
  persisted transaction with no recorded terminal on-chain outcome** — every
  `TxSubmitted`, and every `MintingFailed` reached from `MintIntended` or
  `TxSubmitted`, is such a state. The check reads one instant, while a pending
  or replaceable transaction can still land afterwards; since `MintClosed`
  removes the mint from recovery and from stuck queries, that later landing
  would go unobserved while the operator starts a replacement issuance. For
  those states `CloseMint` therefore additionally requires **positive proof that
  the persisted transaction can no longer land** — either a confirmed receipt
  showing it reverted, or the signing wallet's on-chain nonce confirmed past
  that transaction's nonce by a different transaction (dropped or replaced).
  Without such proof for every unresolved persisted transaction in the history,
  `CloseMint` is rejected with an actionable error and the mint stays open for
  reconciliation. A mint classified `NonceReplayUnresolved` is a separate
  rejection with its own escape hatch — the bot cannot obtain that proof itself
  there, so closing requires `acknowledged_unresolved_mint_nonce` to echo the
  mint's persisted nonce exactly, recording that an operator verified the
  absence against a chain view outside this bot (see "Recipient Authorization"
  -> "Nonce"). The parameter is rejected on any other mint. Emits `MintClosed`.
  Closed mints are excluded from recovery and stuck queries
...
- `OrchestratorMintRecovered` (orchestrator mode only) - Existing on-chain
  ...
  vault-direct `Recover`'s proactive receipt-inventory check (see "Recover"
  above). A `NonceReplayed` revert is only the fallback signal for a
  submit/query race, not the primary discovery path. Carries
...
| `AuthorizeMint`                   | `MintAuthorizationReceived`                                                | Orchestrator mode only. Associates the liquidity bot's validated `MintAuthV1`; no lifecycle transition                                                                                                                                                                                                                    |
...
| `CloseMint`                       | `MintClosed`                                                               | Admin close from pre-mint states only, guarded against an already-landed on-chain mint                                                                                                                                                                                                                                    |
| `ConfirmMint` (orchestrator mode) | `OrchestratorTokensMinted` / `MintingFailed` / `OrchestratorMintRecovered` | New success event; a `NonceReplayed` revert applies the full-match check from "Nonce" below, yielding `OrchestratorMintRecovered` on a match, `MintingFailed` (`NonceConsumedByOtherMint`) on a `Minted` log that disagrees on `token`/`amount`, or `MintingFailed` (`NonceReplayUnresolved`) when no log is found at all |
| `Recover` (orchestrator mode)     | See below                                                                  | Existing-mint check queries the orchestrator's `Minted` log instead                                                                                                                                                                                                                                                       |
...
- `ForceCompleteBurn { issuer_request_id, burn_tx_hash, block_number, reason,
  ...
  block number. The receipt reservation is settled (mirror reduced) just like a
  normal burn completion. Emits `BurnForceCompleted`, transitioning to
  `Completed`. The persisted bytes must decode with matching hash and nonce and
  recover the configured bot wallet as signer; the supplied hash must then equal
  that exact transaction hash unless the operator explicitly acknowledges the
  persisted hash with `acknowledged_unresolved_burn_tx_hash`. A different
  proving hash is rejected by default while the persisted transaction may still
  land. The acknowledgement must equal the persisted hash exactly and is
  recorded in the terminal event. The alternate transaction's mode-specific
  calldata and resulting transfers (for vault-direct: the per-receipt
  withdrawals, recipient wallet, and dust share transfer) must also match the
  persisted burn semantics exactly, including the aggregate burned-share total.
  Its signer nonce must equal the persisted transaction's nonce, proving it is a
  mined replacement rather than an unrelated burn and ensuring the acknowledged
  transaction can no longer land. This prevents another redemption's same-vault
  burn from being used as proof. A `Failed` redemption that still carries a
  persisted signed burn is held to the same binding and acknowledgement rules. A
  legacy `Failed` redemption with **no** persisted signed transaction — a
  custodian-era burn identified only by a backend transaction id the current
  backend cannot look up — is force-completed offline via
  `issuer force-complete-redemption`: the operator supplies the on-chain hash,
  and the CLI proves it is a successful burn on the redemption's vault whose
  per-receipt withdrawals match the burn plan persisted by the latest
  `BurningFailed` event exactly, with the owner recovered from the transaction's
  own signature, and refuses a hash any other redemption's history already
  mentions (custodian-era burns predate orchestrator mode, so the CLI's
  vault-direct proof shape is the only one that applies). Pre-intent states with
  no persisted burn plan at all are **not** force-completed; ops use
  `CloseRedemption` after off-chain reconciliation instead.
...
| Command                                  | Events                             | Notes                                                                                          |
| ---------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------- |
| `DetectRedemption`                       | `RedemptionDetected`               | Transfer detected; captures `burn_mode` for later mode derivation                              |
...
orchestrator-mode events above. This differs from the Mint side, where
`SubmitMint`/`ConfirmMint` already carry only `issuer_request_id`-shaped
scalars, so "reused unchanged" holds exactly — on the Redemption side,
`BurnTokens`'s `vault`/`burns`/ `dust_shares`/`owner` fields and
...
so recovery has nothing to reconcile on-chain for it: `Recover` skips the
...
- **Inconclusive lookup.** _No_ `Minted` log at `(to, nonce)` was found at all,
  ...
  **unknown outcome, not proof of anything**, and must never be recorded as
  ...
  forward through `Recover` once the log is visible again. If they confirm the
  mint genuinely never landed, they close it with an explicit acknowledgement
  parameter on `CloseMint` — mirroring the redemption side's
...
`ConfirmMint` applies this same full-match check, and the same two-outcome
split, when it is the one to observe a `NonceReplayed` revert on a submitted
transaction, rather than `Recover`.
...
sequenceDiagram
    ...
    alt halted (VaultLogicMismatch / ReceiptLogicMismatch)
        ...
        alt reverts (BadRecipientSignature / RecipientCallbackRejected /<br/>VaultAmountMismatch)
            Note right of Us: SubmitMint/ConfirmMint command<br/>Event: MintingFailed (classified, not auto-retried)
        else reverts (NonceReplayed)
            Note right of Us: ConfirmMint applies the full-match check (to, nonce, token, amount)<br/>Match: Event OrchestratorMintRecovered. Log found but token/amount disagree:<br/>Event MintingFailed (classified NonceConsumedByOtherMint).<br/>No log found at all: Event MintingFailed (classified NonceReplayUnresolved)
        else reverts (VaultLogicMismatch / ReceiptLogicMismatch - post-hoc race)
            Note right of Us: SubmitMint/ConfirmMint command<br/>Event: MintingFailed (classified,<br/>attempts NOT advanced, auto-resumes once healthy)
        else succeeds
            Orchestrator->>Us: Minted(token, wallet, amount, nonce)
            Note right of Us: SubmitMint/ConfirmMint command<br/>Event: OrchestratorTokensMinted
            Us->>Alpaca: POST /tokenization/callback/mint<br/>{tx_hash, wallet_address}
            Note right of Us: SendCallback command<br/>Event: MintCompleted
...
sequenceDiagram
    ...
    Note right of Us: DetectRedemption command<br/>Event: RedemptionDetected
...
#### `VaultLogicMismatch` / `ReceiptLogicMismatch` — orchestrator-wide halt

Revert when the production vault or receipt beacon was upgraded ahead of the
orchestrator's expectations. This is a global halt condition, not a
per-operation failure, and must not consume any aggregate's retry budget —
whether the halt is caught before submission or discovered as a revert on a
...
- **Pre-submit health gate (no submission, no event):** before submitting any
  ...
  "orchestrator halted" is a global condition, not a domain fact about any one
  mint or redemption.
- **Post-submit revert (transaction was submitted, then reverts):** if the
  ...
  **either** `VaultLogicMismatch` or `ReceiptLogicMismatch` — `mint()` writes
  ...
  a submitted transaction: `ConfirmMint`/`ConfirmBurn` records it via the
  ...
  advances on every failure" rule stated under "Mint Aggregate" -> `Recover`, a
  `VaultLogicMismatch`- or `ReceiptLogicMismatch`-classified `MintingFailed`
  does **not** advance the `attempts` counter, mirroring the pre-submit case's
  no-counter-advance treatment (burn failures have no analogous automatic-retry
  counter to begin with — see `InsufficientReceipts` above). Recovery resumes
  the same way once `vaultLogicIsExpected()` reports healthy again: for mint,
  the next scheduled or manual `Recover` attempt resubmits the deterministic
  retry (same `external_tx_id` scheme) without having consumed a retry slot; for
  burn, the existing manual `ResumeBurn` re-drive resumes it, exactly as it does
  for `InsufficientReceipts`/`AllowanceInsufficient`.
...
- `BadRecipientSignature` and `RecipientCallbackRejected` are both deterministic
  ...
  **cannot be resumed on the same aggregate**: the manual admin reprocess path
  (`POST /admin/reprocess/mint/<aggregate_id>` -> `Recover`) only resubmits the
  same stored `mint_authorization`/nonce (see "Recipient Authorization" ->
  "Nonce" above — the authorization is fixed per mint and persisted on
  `MintAuthorizationReceived` before the first submission), which would fail
  identically. The only resolution is `CloseMint` on the stranded aggregate,
  followed by a brand-new `Initiate` request (through the normal ITN flow),
  paired with fresh authorization from the liquidity bot via the internal
  mint-authorization call. (Manual reprocess/`Recover` remains the correct
  re-drive path for _transient_ on-chain failures where the same authorization
  is still valid — it is only these two deterministic authorization reverts that
  need a fresh authorization instead.)
- `VaultAmountMismatch` means the orchestrator's on-chain 1:1 assertion failed —
  ...
  (`POST /admin/reprocess/mint/<aggregate_id>` -> `Recover`), which resubmits
  the identical `mint()` call under the same stored authorization/nonce; a fresh
  `Initiate` is not required.
...
stateDiagram-v2
    ...
    MintingFailed --> CallbackPending: Recover, orchestrator mode (OrchestratorMintRecovered)
    TxSubmitted --> CallbackPending: ConfirmMint on NonceReplayed full-match, orchestrator mode (OrchestratorMintRecovered)
    CallbackPending --> Completed: SendCallback
    PendingJournal --> Closed: CloseMint (admin)
    JournalConfirmed --> Closed: CloseMint (admin)
    Minting --> Closed: CloseMint (admin)
    TxSubmitted --> Closed: CloseMint (admin)
    MintingFailed --> Closed: CloseMint (admin)
    JournalRejected --> [*]
    Completed --> [*]
    Closed --> [*]
```
...
- `422`: Invalid state transition (`CallbackPending`, or a matching on-chain
  mint was found — reconcile via `Recover` instead, or a persisted transaction
  is still unresolved, or the mint is classified `NonceReplayUnresolved` and no
  matching `acknowledged_unresolved_mint_nonce` was supplied)
