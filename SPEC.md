...
seconds, meaning nothing happened (no exclusion written, nothing signed) and the
request can simply be retried; 504 when the run exceeds 120 seconds, meaning the
poller has resumed but the outcome is unknown (the burn may be excluded,
intended, or submitted), so the operator re-invokes the route for the same
deposit to read the persisted stream and resume it from wherever it stopped.
