...
- `RecordTxSubmitted { issuer_request_id, external_tx_id, tx_id }` - Records a
  successful on-chain submission performed by `SubmitMintJob`. Pure, accepts
  `MintIntended` on the current job path and `Minting` only for compatibility
  with historical pre-intent submissions, and emits `MintTxSubmitted`
- `RecordTokensMinted { issuer_request_id, tx_id, tx_hash, receipt_id, shares_minted, gas_used, block_number }` -
...
- `RecordMintFailed { issuer_request_id, error }` - Records a side-effect
  failure reported by a job. Pure, emits `MintingFailed`. A failure from
  `MintIntended` retains that state (including its prepared transaction) in the
  failure history, so `RetryMint` and the next `SubmitMintJob` rebroadcast the
  same persisted bytes rather than prepare a replacement
...
event commit (ADR-0001). The handlers above are pure: each job owns one durable
stage, performs the external interactions needed for that stage, and reports
state-changing outcomes via the matching `Record…` command. The job chain is
`SubmitMintJob` (prepares and signs the transaction, persists `MintTxIntended`,
then calls `vault.submit_mint`) -> `ConfirmMintJob` (polls `vault.confirm_mint`,
registers the receipt) -> `SendCallbackJob` (calls Alpaca);
`process_journal_completion` resolves the vault and enqueues the first job.
`SubmitMintJob` uses the configured network's Turnkey-backed vault service to
prepare and submit the signed transaction. A retry with an unresolved
`MintIntended` predecessor — including one retained through `MintingFailed` and
`RetryMint` — rebroadcasts those exact persisted bytes instead of preparing a
replacement, and other mint jobs wait while that wallet intent is unresolved.
...
| `Deposit`            | `MintingStarted`        | Records business intent                                               |
| `RecordTxIntended`   | `MintTxIntended`        | Persists signed tx before broadcast                                   |
| `RecordTxSubmitted`  | `MintTxSubmitted`       | Current path follows `MintTxIntended`; `Minting` is legacy-compatible |
...
`Deposit` emits only `MintingStarted` (business intent). `SubmitMintJob` then
prepares and signs the exact transaction and sends `RecordTxIntended`,
persisting the bytes in `MintTxIntended` before any broadcast. A successful
broadcast is recorded separately through `RecordTxSubmitted`; a preparation or
submission failure is reported through `RecordMintFailed`. A crash after
`MintTxIntended` causes recovery to rebroadcast the same bytes rather than sign
a replacement. Preparing, persisting, and initially broadcasting share one
wallet critical section, and new preparation is blocked while another wallet
...
for the mint's current state — `SubmitMintJob` from `Minting` or `MintIntended`,
`ConfirmMintJob` from `TxSubmitted`, `SendCallbackJob` from `CallbackPending` —
or issues the pure transition that precedes it (`Deposit` from
`JournalConfirmed`, `RetryMint` from `MintingFailed`). The startup re-scan, the
periodic reconciler, the receipt monitor (on ITN-receipt discovery), and the
admin reprocess endpoint all route through the same enqueue path.

Re-submission is double-mint-safe: before preparing or broadcasting,
`SubmitMintJob` checks the receipt inventory and, if the mint already succeeded
on-chain, sends `RecordExistingMint` (emitting `ExistingMintRecovered`) instead.
A pre-existing `MintIntended` state is recovered by broadcasting its persisted
signed transaction. The retry-delay/exhaustion schedule is driven by a
`MintingFailed` attempt counter (retries due after 1m, 10m, 30m, 1h, then
exhausted); a discovered receipt bypasses the backoff so a mint that actually
succeeded is recorded immediately.
...
sequenceDiagram
    ...
    Us->>Us: Prepare and sign exact transaction
    Note right of Us: RecordTxIntended command<br/>Event: MintTxIntended
    ...
    rect rgb(200, 220, 250)
        ...
        Note right of Us: RecordTxSubmitted command<br/>Event: MintTxSubmitted
...
stateDiagram-v2
    ...
    Minting --> MintIntended: RecordTxIntended (MintTxIntended)
    Minting --> TxSubmitted: RecordTxSubmitted (MintTxSubmitted, legacy compatibility)
    Minting --> CallbackPending: RecordExistingMint (ExistingMintRecovered)
    Minting --> MintingFailed: RecordMintFailed (MintingFailed)
    MintIntended --> TxSubmitted: RecordTxSubmitted (MintTxSubmitted)
    MintIntended --> CallbackPending: RecordExistingMint (ExistingMintRecovered)
    MintIntended --> MintingFailed: RecordMintFailed (MintingFailed)
    ...
    PendingJournal --> Closed: CloseMint (MintClosed)
